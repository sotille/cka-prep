...
- **`system:masters` is a hardcoded superuser group** in the API server — it bypasses authorization entirely, no RBAC binding needed, and cannot be revoked except by reissuing/rotating certs. This is exactly the username-vs-groups split above: in kubeadm's `admin.conf`, `kubernetes-admin` is the certificate **CN** (the username), while the **O** field is the group. Historically that O was `system:masters` (hence admin.conf had unrevocable superuser rights); since v1.29 kubeadm split the credentials — `admin.conf` now carries **O=kubeadm:cluster-admins** (bound to `cluster-admin` via an ordinary RBAC ClusterRoleBinding, so it *is* revocable), and **O=system:masters** now lives only in `super-admin.conf`. Never sign user certs with O=system:masters.
